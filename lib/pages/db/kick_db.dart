...
class KickDb {
  ...
  Future<List> list(String today) async {
    var dbClient = await db;
    var result = await dbClient!.rawQuery('SELECT * FROM (SELECT * FROM $tableName '
        'WHERE SUBSTR($columnTime, 1, 11) = "$today" ORDER BY $columnTime DESC LIMIT 10) ORDER BY $columnTime');
    return result.toList();
  }
