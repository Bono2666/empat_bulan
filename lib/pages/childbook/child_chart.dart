...
class _ChildChartState extends State<ChildChart> {
  List dbNotifications, dbChildBook, dbProfile, dbZScore, dbZScoreAge, dbRangeChildBook;
  bool firstLoad = true;
  int ageMonth, ageYear, startMonth;
  double weightUnder1, weightUnder6, weightUnder12;
  String status, chartType, sex, chartTitle, statusDescription;
                              ...
                              builder: (context, snapshot) {
                                ...
                                if (snapshot.connectionState == ConnectionState.done) {
                                  ...
                                  if (double.parse(dbChildBook[dbChildBook.length - 1]['weight']) < double.parse(dbZScoreAge[0]['weight_min2'])) {
                                    status = "Kurang";
                                    statusDescription = 'Perhatikan asupan gizi si kecil untuk mendukung perkembangan tubuhnya. Pantau ulang berat badannya secara berkala.';
                                  } else if (double.parse(dbChildBook[dbChildBook.length - 1]['weight']) <= double.parse(dbZScoreAge[0]['weight_plus2'])) {
                                    status = "Normal";
                                    statusDescription = 'Berat badan anak sesuai umur. Pantau ulang berat badan secara berkala.';
                                  } else {
                                    status = "Obesitas";
                                    statusDescription = 'Perhatikan aktivitas dan istirahat serta asupan si kecil. Pantau ulang berat badannya secara berkala.';
                                  }
                                }
                                return Stack(
                                  children: [
